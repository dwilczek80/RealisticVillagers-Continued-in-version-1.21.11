package me.matsubara.realisticvillagers.entity.v26_1.villager.ai.behaviour.core;
...
public class MoveToTargetSink extends Behavior<Villager> {
    ...
    /** Ticks to wait after being stuck before retrying. */
    private int stuckCooldown;
    /** The last computed path we handed to the navigator. */
    private @Nullable Path path;
    /** Block position of target when we last computed the path. */
    private @Nullable BlockPos lastTargetPos;
    /** Speed modifier taken from the WalkTarget. */
    ...
    public boolean checkExtraStartConditions(ServerLevel level, Villager villager) {
        // Suppress movement while shaking head (e.g. refusing trade).
        if (villager instanceof VillagerNPC npc && npc.isShakingHead()) return false;

        // Wait out stuck cooldown before retrying.
        if (stuckCooldown > 0) {
            stuckCooldown--;
            return false;
        }
        ...
        // Already close enough — clean up and don't start.
        ...
        // Try to find a path. On success, save the target position and proceed.
    ...
    public void tick(ServerLevel level, Villager villager, long time) {
        ...
        // Keep PATH memory in sync with the live navigation path.
        ...
        // Recompute path when the target has moved more than ~2 blocks.
        if (currentTargetPos.distSqr(lastTargetPos) > 4.0) {
            if (computePath(villager, target, level.getGameTime())) {
                lastTargetPos = currentTargetPos;
                start(level, villager, time); // Apply new path immediately.
            }
        }
    }
    ...
    public void stop(ServerLevel level, @NotNull Villager villager, long time) {
        ...
        // If we stopped because we're stuck (not because we arrived), apply a
        // cooldown so the villager doesn't thrash the pathfinder every tick.
        ...
        // We only erase the target in checkExtraStartConditions when reached.
    ...
    /**
     * Attempts to compute a navigable path to the walk target.
     *
     * Critically, uses {@code target.getCloseEnoughDist()} as the path reach
     * so the pathfinder can succeed with a nearby endpoint rather than requiring
     * an exact match (reach=0), which fails in almost all real terrain.
     */
    private boolean computePath(Villager villager, @NotNull WalkTarget target, long gameTime) {
        ...
        // --- Primary attempt: path with reach = closeEnoughDist ---
        int reach = target.getCloseEnoughDist();
        path = getNavigation(villager).createPath(targetPos, reach);

        // --- Evaluate result ---
        if (path != null && path.canReach()) {
            // Perfect — full reachable path found.
            brain.eraseMemory(MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE);
            return true;
        }

        // Record the first time we couldn't fully reach the target.
        ...
        // Partial path (exists but path.canReach() == false) — use it anyway.
        // The villager will get as close as possible, which is much better than standing still.
        if (path != null) return true;

        // --- Fallback: find a navigable position in the direction of the target ---
