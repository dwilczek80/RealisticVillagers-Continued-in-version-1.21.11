...
public class VillagerFoodData {
    ...
    public void tick() {
        ...
        if (naturalRegeneration && saturationLevel > 0.0f && npc.getHealth() < npc.getMaxHealth() && foodLevel >= 20) {
            ++tickTimer;
        ...
        } else if (naturalRegeneration && foodLevel >= 18 && npc.getHealth() < npc.getMaxHealth()) {
            ++tickTimer;
    ...
    private @NotNull VillagerFoodLevelChangeEvent callEvent(int level, @Nullable ItemStack item) {
        VillagerFoodLevelChangeEvent event = new VillagerFoodLevelChangeEvent(
                npc,
                level,
                item != null ? CraftItemStack.asCraftMirror(item) : null);
